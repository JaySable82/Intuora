...
const Kart = ({ cart=[], updateCart, disableOrder,setDisableOrder }) => {
    ...
    const handlePlaceOrder = async () => {
       ...
       if(disableOrder || !kitchenActive) return;
        ...
        try {
            ...
            const response = await fetch(`${AWS_URl}/admin/cart`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    cart: cart.map((item, index) => ({
                        ...
                        parcel: selectedItems[index] || false, // Ensure parcel status is included
                        bottle:bottle
                    })),
                    total: getTotal(),
                    tableNo: tableNo_c,
    ...
    const checkKitchenStatus = () => {
        ...
        // Kitchen is open between 11:00 AM today and 2:00 AM the next day
        const isOpen =
            (currentHours === 8 && currentMinutes >= 0) || // From 11:00 AM to 11:59 AM today
            (currentHours > 8 && currentHours < 24) || // All hours between 12:00 PM and 11:59 PM today
            (currentHours === 0) || // From 12:00 AM to 1:59 AM the next day
            (currentHours === 1) || 
        ...
        useEffect(() => {
            checkKitchenStatus(); // Check kitchen status on mount
